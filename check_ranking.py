...
RARE_KEYWORDS = [
    ...
    "ベイブレード", "BEYBLADE",   # 2026-07-09 相棒追加: 少年層で再ブーム中
    "ハンディ扇風機", "ハンディファン",  # 夏の定番急上昇（季節モーメントとも連動）
    ...
    # 【2026-05-26】 "コーヒー" だと "WPC コーヒー味プロテイン" 等の風味系も誤検出するため
    # "コーヒー豆" に変更。実際の珈琲商品だけ拾う。
    "コーヒー豆",
    # イベント・抽選系
    "抽選販売", "予約受付", "先行予約",
]
...
# 並び順=優先度。より具体的・誤マッチ少ない語を上に置く。
# IP cooldown では「最初にマッチした IP key」で4日間ロックされるので、
# 個別作品IPを上に、ハードウェア総称IPを下に配置すると granular な制御になる。
IP_HASHTAGS = [
    # ── 漫画・アニメ IP（個別作品ごと、新刊/グッズで急上昇しがち） ──
    (["鬼滅の刃", "鬼滅"],                      "#鬼滅の刃"),
    ...
    (["ハイキュー"],                            "#ハイキュー"),
    (["葬送のフリーレン", "フリーレン"],         "#葬送のフリーレン"),
    (["薬屋のひとりごと"],                       "#薬屋のひとりごと"),
    (["キングダム"],                            "#キングダム"),
    (["ガンダム", "ガンプラ"],                  "#ガンダム"),
    (["進撃の巨人"],                            "#進撃の巨人"),

    # ── ゲームソフト個別IP（同じ Switch でも franchise ごとに別cooldown） ──
    (["ゼルダの伝説", "ゼルダ", "ティアキン"],   "#ゼルダの伝説"),
    (["スーパーマリオ", "マリオカート", "マリオ"], "#スーパーマリオ"),
    (["どうぶつの森", "あつ森"],                "#どうぶつの森"),
    (["スプラトゥーン"],                        "#スプラトゥーン"),
    (["スマブラ"],                              "#スマブラ"),
    (["ファイアーエムブレム"],                  "#ファイアーエムブレム"),
    (["ポケモンレジェンズ", "ポケモンSV", "ポケモン スカーレット"], "#ポケモンSV"),

    # ── TCG / カードゲーム ──
    (["ポケモンカード", "ポケカ"],              "#ポケモンカード"),
    (["遊戯王"],                                "#遊戯王"),
    (["マジック：ザ・ギャザリング", "MTG"],     "#MTG"),

    # ── 推し活・コレクター ──
    (["たまごっち", "Tamagotchi"],              "#たまごっち"),
    (["ベイブレード", "BEYBLADE"],              "#ベイブレード"),
    (["ハンディ扇風機", "ハンディファン"],       "#ハンディファン"),
    (["ちいかわ"],                              "#ちいかわ"),
    (["サンリオ"],                              "#サンリオ"),
    (["ドロップシール"],                        "#ドロップシール"),
    (["プチドロップ"],                          "#プチドロップ"),
    ...
    (["ポケットモンスター", "ポケモン"],        "#ポケモン"),
    (["Switch2", "Nintendo Switch", "Nintendo"], "#NintendoSwitch"),
    ...
    # ── 趣味枠 ──
    (["DRIP COFFEE FACTORY"],                   "#DRIPCOFFEEFACTORY"),
    (["コーヒー"],                              "#コーヒー"),
]
