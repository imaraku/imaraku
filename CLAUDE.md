...
### ⚠️ 19. 終了済みキャンペーンのリンクを投稿してしまう（信頼を一発で壊す）
2026-06-10 のトラベル投稿が、リンクカードに「この企画は終了しました」とデカデカ表示される
終了ページ（アーリーサマーフェア）を案内していた。ハードコード/手動キュレートURLを投稿する
スクリプトに「リンク先の生存チェック」が無かったため。

✅ **対策**: `link_guard.py`（共有モジュール）を新設。`is_link_alive(url)` が
  hb.afl ラップ展開→200確認→終了文言検出を行う。**外に出す推薦リンクは「確認できたものだけ
  出す」**（check_campaigns の「不明は現状維持」とは思想が逆。用途が違う）。
  travel は `filter_alive()` 統合済み。monthly_pay は既存 fetch_campaign_info が同等機能。
  **新しい投稿スクリプトでキャンペーンURLを載せる時は必ず link_guard を通すこと。**
✅ **罠**: 楽天系は charset 未宣言ページがあり、requests が ISO-8859-1 と誤推定すると
  本文が文字化けして「終了」がマッチしない（実テストで検出）。`r.apparent_encoding` で
  補正する処理が link_guard / fetch_campaign_info には入っている。自前fetchを書く時は注意。

### ⚠️ 20. キーワード照合は表記ゆれ（カタカナ/ひらがな/全半角）で裏切られる
2026-06-10 に「🍞パン ランキング1位」投稿が**パンパースのオムツ**を紹介した。
原因は二重: ①blacklist「おむつ」(ひらがな)が商品名「オムツ」(カタカナ)に不一致
②そもそも「パンパース」「シャンパン」「フライパン」が blacklist 未登録（whitelist「パン」が貪欲）。

✅ **対策**: post_category_ranking に `_norm()`（NFKC＋小文字化＋カタカナ→ひらがな）を新設し、
  whitelist/blacklist 照合は**必ず正規化済み同士**で行う。パンの blacklist も拡充済み。
✅ **教訓**: 日本語の包含判定を書くときは常に表記ゆれを疑う。「短い語のwhitelist」は
  貪欲マッチ（パン⊂パンパース）するので、blacklist とセットで設計すること。

### ⚠️ 21. py_compile は import 欠落(NameError)を検出しない — 「守るための修正」が最強日を殺した
2026-07-05 に daily-tweet へ280字保険（`_weighted_len`/`_fit_within_limit`、7/3-7/4の
280字超過403事故の恒久対策）を追加した際、`re.sub` を使うのに **import re を書き忘れ**、
スロット内の全runが投稿直前に NameError で即死。**7/5(最強日)〜7/8の4日間、日次投稿が全滅**した
（13run failure。mega-chance/sale-picks/ランキング/カテゴリは正常だったため気づきにくかった）。

【なぜ事前検証をすり抜けたか】
- py_compile は構文しか見ない。NameError は実行時にしか出ない
- 「関数を抽出して別namespaceでテスト」する検証方式は、テスト側が偶然 `re` を持っていて
  依存importの欠落を隠した
- 失敗stepの所要時間 **0秒** が「ネットワーク前の即死＝コード起因」の決定的サイン
  （API拒否ならリトライで数秒かかる）

✅ **定石（投稿スクリプトを変更したら必須）**:
1. **ASTベースの未定義名スキャン**: import/def/代入で定義された名前 vs 使用名を突き合わせ
   （lambda引数は偽陽性になるので注意）
2. **実モジュール全文execのエンドツーエンドsim**: `from __future__ import annotations` を
   先頭に付けて全文exec（py3.9でも `X | None` 注釈を無害化できる）→ namespace の
   `datetime` を FakeDT（時刻偽装）に差し替え → `_post_once` をスタブ → `main()` を実行。
   スロット内時刻・ピーク日・通常日の複数日付で「クラッシュせず投稿(stub)まで到達」を確認
3. 抽出テストは「ロジックの正しさ」用。「モジュールとして動くか」は全文execでしか分からない

### ⚠️ 22. 単独「終了しました」はクーポンボタンの状態ラベルに誤マッチする — 5ヶ月間クーポン2枚が消えていた
2026-09-18 の総ざらい監査で、returnpurchaser（久しぶり1,000円）/ newpurchaser（初めて2,000円）が
**2026-04 から約5ヶ月ずっと false**（サイト非表示）だったと判明。実ページは開催中だったのに、
end_kw の単独「終了しました」が、クーポンボタンの全状態テンプレ（取得済/終了 等がHTMLに常駐）に
マッチしていた。workflow は毎回 success なので誰も気づかない。
✅ **対策**: end_kw は STRICT 句（「本キャンペーンは終了」「このキャンペーンは終了」「受付は終了」）
  のみ＋ `period_check: True`（ページ明記の開催期間で判定）。地雷#1・#18 と同根＝
  **「ページにこの言葉があるか」で状態を決めない。期間表記が最も確実。**
✅ **教訓**: false のフラグは「表示されていない」ため気づきにくい。**定期的な総ざらい監査**
  （4方向: 全リンク生存／フラグと実態の整合／新エントリー発掘／技術点検）を
  少なくともマラソンごとに1回、Workflow の並列エージェントで回すこと。

### ⚠️ 23. date-coded URL と a.r10.to 短縮は「サイレント旧ページ」になる
mobiledeal/20260509/ や books の index-20260424-…-pre.html のように URL に日付が入るページは、
終了文言を出さずに旧回の内容で 200 を返し続ける（=キーワード判定も link_guard も素通り）。
2026-09-18 監査で、おすすめ表示中の楽天モバイル×DEAL が5月ページに、ブックス条件達成が
4月予告ページに誘導していた。旧URL自体が消えた例も: /ichiba-day/ /wonderful-day/ /osechi/（404）。
✅ **対策**:
  - date-coded は dynamic_urls.json で自動追従させる。mobiledeal は「superdeal トップに無ければ
    marathon_token の先頭8桁(=開始日)で probe」のフォールバック済み。ブックスはハブURL＋
    `campaignKey: marathon_pointup`（期間外はハブが終了表示になるため）
  - a.r10.to は必ず展開して生URLで載せる（expired 照合・生存判定が効くように）。
    残り: PREMIUM DAYS / 本雑誌 / DVD / kobo初回 の4枚（次回の宿題）
  - 監査時は「200かつ終了文言なし」を信用せず、**本文の開催期間が現在を含むか**まで見る
  - new_campaigns.json の purge にも period_status=='expired' を追加済み（19件残留していた）

### 🍉 補足: 季節モーメント投稿（2026-07-09 相棒発案・ライト層リーチの主力）
ライトユーザーの生活イベント（夏休みの虫取り・花火の浴衣・お盆の手土産等）に
「買う前に30秒エントリー」を届ける仕組み。`seasonal_moments.json` がカレンダー。
- **発火**: daily-tweet の「通常日」filler枠を置き換え（投稿数・クレジット消費は増えない）。
  セール・特別日・勝利など既存の全分岐が優先＝静かな日（月10〜15日）だけ発火する
- **期間**: from/to は MM-DD 形式で**毎年自動再利用**（年跨ぎ非対応）。重複期間は
  「終了が近い方」優先（今言わないと間に合わない方）
- **item回転**: `yday×2＋スロット` ベース。⚠️ `now.day % len` にしないこと
  （len=5 だと index0 が「0と5のつく日」=非発火日に固定され永遠に出ないバグをsimで実測）
- **メンテ**: items に1行足すだけ。**雑談化させない**＝「イベント→買うモノ→エントリー導線」の
  三段構成を厳守（X投稿の原則参照）。次マラソンが10日以内なら「⏳M/D〜はマラソンで更にお得」を自動挿入
- 既存の季節イベント分岐（母の日/父の日/クリスマス当日等 = SEASON_TWEET_MAP）とは別系統で
  共存する（当日ピンポイント告知は既存側・事前の買い物準備期間はモーメント側）

### 📉 補足: 急上昇ランキングのアイドル/音楽枠は 2026-07-09 廃止（掘り起こし禁止）
Snow Man/なにわ男子/K-POP等21アクトを RARE_KEYWORDS と IPマップから除去（相棒の判断）。
インプレッション自体は取れていた（実測 StrayKids 64view=平均超）が、ファンの検索流入は
今楽のブランド（楽天お得情報）に転換しない off-brand トラフィックで、フィードの一貫性を優先。
アニメ/ゲーム/TCG/ガジェット/コーヒーの検出は維持。CD/DVD軸(101240)はアニメBD用に存続。

### 📣 X投稿の原則（2026-06-11 フォロワー戦略レビューで制定・相棒合意）
1. **リンクは1投稿1〜2本まで**。生アフィURL（hb.afl...の長い文字列）の羅列は
   スパムに見える。複数案件はテキスト紹介＋代表リンク1本に集約する。
2. **1行目=価値ファースト**。日付・スロット文脈行（dedup用）は末尾に置く
   （`with_slot_intro` は末尾付与に変更済み。先頭に戻さないこと）。
3. **リンク先は link_guard で生存確認してから載せる**（地雷#19）。
4. 自動投稿は「情報の信頼性」担当。「人格・会話」（リプ参加・実体験ポスト）は相棒の人力
   担当で、自動リプライは spam/BAN リスクのため実装しない。
...
- 🗑️ **「選択したエントリーを開く」(一括オープン＋チェックボックス)は 2026-06-11 廃止**（掘り起こし禁止）。
  FAB に一本化。理由: ポップアップブロッカーで実際は全部開けない/アプリ遷移リンク混在で挙動不安定/
  カードUIスッキリ化（相棒の判断）。該当者除外などグループ判定は data-group 属性を使うこと。
...
| `keirou` | 敬老の日特集（同上・2026-09-18 追加。カード3枚: point/coupon/kuji） | false |
| `oseibo` | お歳暮特集（同上） | false |
| `osechi` | おせち特集（同上。URL は /toshikoshi/pre/、早期予約8/19〜） | false |
