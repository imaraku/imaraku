...
### slot 設計（2026-05-20 改修後）
- **12時スロット** (JST 12:00-15:59): 昼休みの読者向け
- **18時スロット** (JST 16:00-19:59): 通勤・夕方ゴールデンタイム
- ❌ 0時 / 20時スロットは廃止（X APIクレジット節約）
  - 0時はエンゲージメント最低の深夜帯
  - 20時はマラソン中の prime time だったが、18時スロットに priority logic 経由で
    マラソンツイートを集約することで代替（マラソン #1-#3 優先度ツイートは 18時に必ず乗る）
```
...
| daily-tweet | `0 3-10 * * *` | **JST 12-19時帯のみ 8 fire/日**＋slot dedup (2スロット: 12時/18時) |
