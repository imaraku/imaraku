...
- **起動日**（`is_trigger_day`・毎日23時JST起動→対象日だけ実行）:
  ...
  ③ マイルドさんが当日に大型セール系まとめ（マラソン/スーパーセール/ポイントバック祭/大感謝祭等）を新規投稿
...
- **稼働状態**: ⚠️ 現在 `mild-diff.yml` は **`workflow_dispatch` のみ＝自動では動かない**。
  マイルドさんへの連絡（敬意・許可）が済んでから schedule のコメントを外して稼働。
...
├── check_mild_diff.py          ← マイルドさん差分→取りこぼし拾い（手動のみ・未稼働）
└── .github/
    └── workflows/
        ...
        └── mild-diff.yml            ← マイルドさん差分（手動のみ・連絡後にschedule有効化）
```
