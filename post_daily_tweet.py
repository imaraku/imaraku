...
def tweet_zero_five_day() -> str:
    """0と5のつく日 × ふるさと納税アピール。
    2026-09: 10/1 の制度改正（返礼品の地場産品基準の厳格化・自治体経費上限の引き下げ＝
    返礼品の選択肢と還元率が削られる方向）を 9月中だけ告知する（相棒の要望・公式/報道で事実確認済み）。
    楽天ふるさと納税は「お買い物通常ポイント1倍」対象外だが、5と0のつく日特典(+1倍)は有効
    （event.rakuten.co.jp/card/pointday/ で 2026-09-18 確認）。"""
    now = datetime.datetime.now(JST)
    if now.month == 9 and now.year == 2026:
        why = (
            "⚠️ 10/1からふるさと納税の返礼品ルールが厳格化\n"
            "狙いの返礼品は9月中に寄付が安心🎯\n"
        )
    else:
        # 3行だと最長タグ組合せで283字→長さ保険がタグを削るため2行に圧縮（2026-09-18）
        why = (
            "💡 物価高の今こそ早めのふるさと納税が◎\n"
            "年末より品数豊富・人気返礼品の売り切れ回避🎯\n"
        )
    return (
        f"{daily_lead_in()}"
        "📅 今日は0と5のつく日！\n"
