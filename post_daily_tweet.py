...
SLOT_WINDOWS = [
    # 2026-05-23: X が imaraku URL を flag → 連続 403 で reputation 悪化中。
    # 投稿回数を減らして「失敗の連投」を物理的にゼロに近づけ、reputation 回復を優先。
    # 12時スロット (一日2回目) を一旦廃止し、エンゲージメント最大の18時スロット一本に絞る。
    # X reputation が完全回復したら 12時を再開する（CLAUDE.md 地雷#12 メモ参照）。
    ("18", 16, 22),  # JST 16:00-21:59。GitHub cron の遅延着地(20-21時JST)も拾う（地雷#5）。
                     # 6/1・6/2 は cron 遅延で実行が 20-21時JST に着地→旧窓(16-20)の外→投稿ゼロだった。
]
