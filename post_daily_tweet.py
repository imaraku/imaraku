...
SLOT_WINDOWS = [
    # 2026-05-20: X API クレジット節約のため 4 → 2 スロットに圧縮。
    # 削除: 0時（深夜帯、エンゲージメント最低）/ 20時（マラソン中の prime time だったが
    #       18時スロットに priority logic 経由でマラソンツイートを集約することで代替）
    # 残す: 12時（昼休み）/ 18時（通勤・夕方ピーク）
    ("12", 12, 16),
